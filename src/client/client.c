...
void printMessage(const ClientConnectionData *clientConnectionData, char *message, const int bytesReceived);
...
_Noreturn void *receiveAndPrintMessages(void *threadData) {
    ...
    while (1) {
        ...
        if (bytesReceived > 0) {
            printMessage(clientConnectionData, message, bytesReceived);
        }
    }
}
...
void sendMessage(const ClientConnectionData *clientConnectionData) {
    ...
    while (1) {
        ...
        send(clientConnectionData->connectionSocket, formattedMessage, strlen(formattedMessage), 0);
    }
}
